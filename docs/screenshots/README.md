...
### `26-empty-verse.png`, `27-find-scroll.png`, `28-sidebar-narrow.png` — issue-66 Tier 1 UX states

- **Surface:** the Tier 1 fixes from the issue-66 UX audit. **Standalone** —
  sourced from `demo/ux66-wide.tape` (26, 27) + `demo/ux66-narrow.tape` (28)
  via `just ux66-screenshots`, not the tour, so they never churn the rest.
- **State:**
  - `26` — Berean Standard Bible at Matt 17:21 (a critical-text omission):
    the verse body is the italic **`(omitted)`** placeholder instead of a
    bare number (finding #3).
  - `27` — the Find dialog after arrowing well past the first screenful: the
    selected hit stays highlighted **in view** while the `N of M` counter
    tracks it (finding #1).
  - `28` — a ~95-col terminal (below the ~124-col sidebar threshold): the
    sidebar is width-suppressed and the mode pill reads **`NORMAL | NARROW`**
    (finding #5).
- **Before/after:** each carries a pinned `*-before.png` baseline — the
  pre-fix `main` behaviour (bare verse number / selection scrolled off-screen
  / plain `NORMAL` pill + misleading `Tab Hide`). The baselines are captured
  from a `main` build and are **not** regenerated by the tapes (the old
  behaviour can't be reproduced from this tree), mirroring `25-xrefs-fetch-before.png`.
- **Intentional:**
  - `(omitted)` is italic (not dimmed): `light_grey` would fail AA on the
    teal cursor row, so italic carries the "editorial, not scripture" signal.
  - `NARROW` (width-suppressed) is a distinct pill cue from `NOREFS`
    (toggled off) — the whole point of finding #5.

### `29-showcmd.png`, `30-bookmark-toast.png`, `31-goto-error.png` — issue-66 Tier 2 feedback layer

- **Surface:** the Tier 2 fixes (the silent-feedback findings). **Standalone**
  — `demo/ux66-tier2.tape` via `just ux66-screenshots`, not the tour.
- **State:**
  - `29` — a pending count prefix shows in the pill as **`-- NORMAL 5 --`**
    (vim's `showcmd`); before, the `5` was invisible (finding #7).
  - `30` — pressing `b` shows a **`Bookmarked John 3:16`** transient (the
    feedback layer also behind copy `y` and `n`/`N`); before, the add was
    silent (finding #8).
  - `31` — Goto with an unparseable reference shows a red **`Unknown book or
    reference`** error instead of a neutral yellow hint, so `Enter` doing
    nothing is self-explanatory (finding #11).
- **Before/after:** each carries a pinned `*-before.png` baseline (plain
  `NORMAL` pill / no transient / yellow hint), captured from a pre-fix build
  and not regenerated by the tape.
- **Intentional:**
  - The error preview is `hotkey_red`, not yellow: yellow is the
    operative-token slot (the *resolved* reference only), so an error must
    not wear it.
