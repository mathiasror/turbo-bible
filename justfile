...
# Re-render the issue-66 UX-state captures (docs/screenshots/26-31).
# Standalone tapes; the wide Tier 1 tape needs en-bsb in the data dir (the
# empty critical-text verse). The *-before.png baselines are pinned pre-fix
# captures, not regenerated here.
ux66-screenshots:
    cargo build -p turbo-bible --release
